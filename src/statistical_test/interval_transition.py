...
def load_distribution_to_list(distribution_name, dataset_name):
    ...
    category_list = df.to_numpy().flatten().tolist()
...
distribution_name = "/interval_transition"

dataset_list = ["/CoCoPops.csv", "/han.csv", "/POP909.csv"]
...
dataset_1 = "/POP909.csv"

dataset_2_list = ["/CoCoPops.csv", "/han.csv"]
...
for dataset_2 in dataset_2_list:
    ...
    #continue
    diff = dataset_1_arr - dataset_2_arr

    diff = [1 if x==0 else x for x in diff]

    diff_arr_dict[dataset_2] = diff
